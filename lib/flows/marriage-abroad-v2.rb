...
country_select :country_of_ceremony?, :use_legacy_data => true do
  ...
  calculate :country_name_partner_residence do
    if data_query.british_overseas_territories?(ceremony_country)
      ...
      "Dutch"
    elsif ceremony_country == 'hong-kong-(sar-of-china)' or ceremony_country == 'macao'
      "Chinese" 
    else
...
country_select :residency_nonuk?, :use_legacy_data => true do
  ...
  next_node do
    case ceremony_country
    when 'france','new-caledonia','wallis-and-futuna'
      :marriage_or_pacs?
    when 'french-guiana','french-polynesia','guadeloupe','martinique','mayotte','new-caledonia','reunion','st-pierre-and-miquelon'
      :outcome_os_france_or_fot
    else
      :what_is_your_partners_nationality?
    end
...
outcome :outcome_os_consular_cni do
  precalculate :consular_cni_os_start do
    ...
    if resident_of == 'uk'
      case ceremony_country
      when 'italy','portugal','philippines'
        ''
      when 'montenegro'
        phrases << :consular_cni_os_uk_resident_montenegro
      when 'poland','kazakhstan','kyrgyzstan'
        phrases << :consular_cni_os_uk_resident_poland_kazak_kyrg
      else
        phrases << :consular_cni_os_uk_resident_legalisation
      end
    end
    if resident_of == 'uk' and ceremony_country != 'italy' and ceremony_country != 'portugal' and ceremony_country != 'philippines'
      phrases << :consular_cni_os_uk_resident_not_italy_or_portugal
    end
    if resident_of == 'uk' and ceremony_country == 'philippines'
    ...
    if ceremony_country == residency_country and residency_country != 'spain' and residency_country != 'germany' and residency_country != 'italy' or (data_query.non_commonwealth_country?(residency_country) and residency_country != 'ireland' and ceremony_country != residency_country)
    ...
    if ceremony_country != residency_country and resident_of != 'uk' or (ceremony_country == residency_country and ceremony_country != 'germany')
      phrases << :consular_cni_os_not_local_resident_not_germany
    end
    if ceremony_country == residency_country and ceremony_country == 'spain'
      phrases << :spain_os_consular_cni_three
    end
    if data_query.non_commonwealth_country?(residency_country) and residency_country != 'ireland' and residency_country != 'spain' and residency_country != 'germany' or (ceremony_country == residency_country and ceremony_country != 'spain' and ceremony_country != 'germany')
  ...
  precalculate :consular_cni_os_remainder do
    ...
    if resident_of == 'other' and ceremony_country != 'italy' and ceremony_country != 'spain'
    ...
    case ceremony_country
    when 'armenia','bosnia-and-herzegovina','cambodia','iceland','kazakhstan','latvia','luxembourg','slovenia','tunisia','tajikistan'
...
outcome :outcome_cp_france_pacs do
  precalculate :france_pacs_law_cp_outcome do
    phrases = PhraseList.new
    if ceremony_country == 'new-caledonia' or ceremony_country == 'wallis-and-futuna'
      phrases << :fot_cp_all
    end
...
outcome :outcome_cp_consular_cni do
  precalculate :consular_cni_cp_outcome do
    ...
    elsif ceremony_country == 'croatia' or ceremony_country == 'bulgaria' and partner_nationality == 'partner_local'

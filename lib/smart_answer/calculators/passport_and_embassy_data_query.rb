module SmartAnswer::Calculators
  class PassportAndEmbassyDataQuery
    ...
    def initialize(passport_fees: nil)
      @passport_data = self.class.passport_data
      @passport_fees = passport_fees || self.class.passport_fees
    end
    ...
    def find_passport_fee(*path)
      path.reduce(passport_fees) { |result, key| result.fetch(key) }
    end

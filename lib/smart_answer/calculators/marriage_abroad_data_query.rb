module SmartAnswer::Calculators
  class MarriageAbroadDataQuery
    COMMONWEALTH_COUNTRIES = %w(antigua-and-barbuda australia bahamas bangladesh barbados belize botswana brunei cameroon canada cyprus dominica fiji ghana grenada guyana india jamaica kenya kiribati lesotho malawi malaysia maldives malta mauritius namibia nauru new-zealand nigeria pakistan papua-new-guinea samoa seychelles sierra-leone singapore solomon-islands sri-lanka st-kitts-and-nevis st-lucia st-vincent-and-the-grenadines swaziland tonga trinidad-and-tobago tuvalu uganda vanuatu zambia).freeze

    REQUIRES_7_DAY_NOTICE_CEREMONY_COUNTRIES = (COMMONWEALTH_COUNTRIES - %w(brunei)) + %w(ireland rwanda st-lucia).freeze

    BRITISH_OVERSEAS_TERRITORIES = %w(anguilla bermuda british-antarctic-territory british-indian-ocean-territory british-virgin-islands cayman-islands falkland-islands gibraltar montserrat pitcairn-island st-helena-ascension-and-tristan-da-cunha south-georgia-and-south-sandwich-islands turks-and-caicos-islands).freeze

    FRENCH_OVERSEAS_TERRITORIES = %w(french-guiana french-polynesia guadeloupe martinique mayotte new-caledonia reunion st-pierre-and-miquelon wallis-and-futuna).freeze

    FRENCH_OVERSEAS_TERRITORIES_OFFERING_PACS = %w(new-caledonia wallis-and-futuna).freeze

    CEREMONY_COUNTRIES_OFFERING_PACS = [%w(monaco) + FRENCH_OVERSEAS_TERRITORIES_OFFERING_PACS].freeze

    DUTCH_CARIBBEAN_ISLANDS = %w(aruba bonaire-st-eustatius-saba curacao st-maarten).freeze
    ...
    OS_NO_CONSULAR_CNI_COUNTRIES = %w(burundi democratic-republic-of-the-congo mexico saint-barthelemy st-martin).freeze

    OS_MARRIAGE_VIA_LOCAL_AUTHORITIES = %w(argentina costa-rica cote-d-ivoire czech-republic israel liberia madagascar netherlands paraguay senegal taiwan ukraine uruguay).freeze
    ...
    OS_CONSULAR_CNI_IN_NEARBY_COUNTRY = %w(nicaragua).freeze

    OS_AFFIRMATION_COUNTRIES = %w(belgium cambodia ecuador lebanon finland macao mongolia morocco norway peru qatar south-korea united-arab-emirates vietnam).freeze

    CP_EQUIVALENT_COUNTRIES = %w(austria brazil czech-republic denmark ecuador finland germany iceland luxembourg netherlands norway slovenia).freeze

    CP_CNI_NOT_REQUIRED_COUNTRIES = %w(andorra argentina bonaire-st-eustatius-saba burundi liechtenstein mexico new-zealand uruguay).freeze

    CP_CONSULAR_COUNTRIES = %w(bulgaria croatia cyprus guatemala moldova panama venezuela).freeze

    COUNTRIES_WITHOUT_CONSULAR_FACILITIES = %w(argentina aruba bonaire-st-eustatius-saba burundi cote-d-ivoire curacao czech-republic saint-barthelemy slovakia st-maarten st-martin taiwan).freeze

    SS_MARRIAGE_COUNTRIES = %w(australia bolivia dominican-republic estonia germany kosovo mongolia montenegro nicaragua russia san-marino serbia).freeze

    NO_SS_MARRIAGE_COUNTRIES = %w(san-marino seychelles).freeze

    SS_MARRIAGE_COUNTRIES_WHEN_COUPLE_BRITISH = %w(lithuania).freeze

    SS_MARRIAGE_AND_PARTNERSHIP_COUNTRIES = %w(albania cambodia japan peru vietnam).freeze

    SS_ALT_FEES_TABLE_COUNTRY = %w(australia bolivia estonia san-marino serbia seychelles).freeze

    SS_ALT_FEES_TABLE_OR_OUTCOME_GROUP_A = %w(mongolia montenegro nicaragua russia).freeze

    SS_ALT_FEES_TABLE_OR_OUTCOME_GROUP_B = %w(azerbaijan dominican-republic kosovo).freeze

    OS_21_DAYS_RESIDENCY_REQUIRED_COUNTRIES = %w(oman qatar yemen).freeze

    SS_UNKNOWN_NO_EMBASSIES = %w(st-martin saint-barthelemy).freeze
    ...
    CNI_POSTED_AFTER_14_DAYS_COUNTRIES = %w(qatar saudi-arabia united-arab-emirates yemen).freeze

...
module MaternityAnswerLogic
  ...
  def should_implement_materntiy_answer_logic
    context "Everybody" do
      ...
      should "see an error message if clicking next without giving response on a date question" do
        click_on "Get started"
        expect_question "When is your baby due?"
        click_next_step
      ...
      should "see an error message if clicking next without giving a response on a multiple choice question" do
        click_on "Get started"
        respond_with Date.today + 30.weeks
        expect_question "Are you employed?"
        click_next_step
    ...
    context "Employed person" do
      ...
      context "who started 26 weeks before qualifying week" do
        ...
        context "(which is in the future)," do
          ...
          context "changing due date" do
            setup do
              within('.done-questions .done', text: /When is your baby due\?/) do
                click_on "Change this answer"
              end
            end

            should "redisplay the question" do
              expect_question "When is your baby due?"
            end

            should "prefill the date selections" do
              within '.current-question' do
                assert_equal @due_date.day.to_s, find("select[name='response[day]']").value
                assert_equal @due_date.month.to_s, find("select[name='response[month]']").value
                assert_equal @due_date.year.to_s, find("select[name='response[year]']").value
              end
            end
          end

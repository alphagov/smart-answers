...
class RecruitmentBannerTest < ActionDispatch::IntegrationTest
  ...
  def visa_banner
    {
      URL: "https://surveys.publishing.service.gov.uk/s/0DZCPX/",
      path: "/check-uk-visa",
    }
  end
  ...
  context "Visa service recruitment banners" do
    setup do
      stub_content_store_has_item(visa_banner[:path])
      stub_content_store_has_item("/bridge-of-death")
    end

    should "display Recruitment Banner on the landing page of the specific smart answer" do
      visit visa_banner[:path]
      assert page.has_css?(".gem-c-intervention")
      assert page.has_link?("Take part in user research (opens in a new tab)", href: visa_banner[:URL])
    end

    should "not display Recruitment Banner on non-landing pages of the specific smart answer" do
      visit "#{visa_banner[:path]}/y"

      assert_not page.has_css?(".gem-c-intervention")
      assert_not page.has_link?("Take part in user research (opens in a new tab)", href: visa_banner[:URL])
    end

...
module SmartAnswer::Calculators
  class CheckBenefitsFinancialSupportCalculatorTest < ActiveSupport::TestCase
    context CheckBenefitsFinancialSupportCalculator do
      ...
      context "#eligible_for_winter_heating?" do
        context "when eligible" do
          should "be true if benefits are present" do
            @calculator.on_benefits = "yes"
            assert @calculator.eligible_for_winter_heating?
          end
        end
      end

      context "#eligible_for_child_winter_heating?" do
        context "when eligible" do
          should "be true if with children with a disablity" do
            @calculator.children_with_disability = "yes"
            assert @calculator.eligible_for_child_winter_heating?
          end
        end
      end
    end

...
module SmartAnswer
  class DateRangeTest < ActiveSupport::TestCase
    ...
    context 'when range is built with begins_on & duration' do
      setup do
        @date_range = DateRange.new(begins_on: Date.parse('2000-01-01'), duration: 1.week)
      end
      ...
      should 'end on day before begins_on plus duration' do
        assert_equal Date.parse('2000-01-07'), @date_range.ends_on
      end
    end

    context 'when range is built with begins_on, ends_on & duration' do
      setup do
        @date_range = DateRange.new(begins_on: Date.parse('2000-01-01'), ends_on: Date.parse('2000-02-14'), duration: 1.week)
      end

      should 'ignore ends_on and use duration instead' do
        assert_equal Date.parse('2000-01-07'), @date_range.ends_on
      end
    end

    context 'when range is built with no ends_on or duration' do
      setup do
      ...
      should 'inlcude date far in the future' do
        assert @date_range.include?(Date.parse('4000-01-01'))
      end
    ...
    context 'when range includes leap day' do
      setup do
        @date_range = DateRange.new(begins_on: Date.parse('2000-01-01'), duration: 1.year)
      end
      ...
      should 'end on ends_on date' do
        assert_equal Date.parse('2000-12-31'), @date_range.ends_on
      end

...
class FlowPresenter
  ...
  def previous_state
    return if current_state.path.empty?
  ...
  def back_link
    if previous_state.nil?
      ...
      smart_answer_path(
        id: @params[:id],
        started: "y",
        responses: accepted_responses[0...current_question_number - 2],
        previous_response: accepted_responses[current_question_number - 2],
      )
    end
  end

...
These tests were introduced by @chrisroos & @jamesmead in 2015 to reduce the risk of doing large-scale refactoring within the application. In particular, they wanted to make a substantial change to the way that landing, question & outcome pages were rendered. The vast majority of the integration tests did not (and still don't) render these pages and in general test coverage was very patchy. The regression tests were created to fulfil this requirement.
...
2. Run the regression tests thus re-generating the artefacts
3. Review the changes to the artefacts and check they are as you expect
4. If artefact changes are as expected, re-generate the checksums
